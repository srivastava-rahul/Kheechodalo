...
<div class="body-area">
    ...
    <input type="text" id="myInput" onkeyup="myFunction()" placeholder="Search By Email..." title="Type in Email" style = "width: 452px; height: 38px;;margin-left: 25%"/>
		...
		<c:forEach var="win" items="${winner}" varStatus="index">
			<tr>
...
<c:if test="${empty searchPic}">
       <c:forEach var="win" items="${winner}">
			<div class="mySlides">
					...
					<div class="card">
					<img src="data:image/jpeg;base64,${win.base64Encoded}"	style="width:100%;height:90%">
					<h2><span style="margin-left: 50%;color: red; " class="voteCount${pic.id}">${win.picVote}</span></h2>
					<div class="">
						<b><h2 style="color:blue"><a href="${pageContext.request.contextPath}/user/viewProfile/${win.email_id}">${pic.userName}</a></h2></b>
						<%-- <b><h2 style="color:blue">${pic.userEmailId}</h2></b> --%>
						<center><p style="color:green;">${win.winner_desc}</p></center>
					</div>
				</div>
				</div>
